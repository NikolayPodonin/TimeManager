...
class CategoryHolder extends RecyclerView.ViewHolder
        implements RvCategoriesAdapter.OnSetActiveHolderListener {
    ...
    public void bind(RvCategoriesAdapter.CategoryDraws categoryDraws, int position, final ActivePositionSetter setter){
        ...
        mImageButton.setOnClickListener(v -> setter.setActivePosition(mPosition));
    }

    @Override
    public void onSetActiveHolder(int position) {
        if(mPosition == position){
            mImageButton.setImageResource(mCategoryDraws.getColorDraw());
            mTextView.setTextColor(mTextView.getContext().getResources().getColor(mCategoryDraws.getColorResource()));
        } else {
            mImageButton.setImageResource(mCategoryDraws.getBlackDraw());
